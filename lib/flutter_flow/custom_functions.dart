...
DateTime? oneDayAwayFromToday() {
// Get the current date
  DateTime now = DateTime.now();

  // Add one day to the current date
  DateTime oneDayAway = now.add(Duration(days: 1));

  return oneDayAway;
}

DateTime? sevenDaysBefore(DateTime? inputDateTime) {
  DateTime? sevenDaysAgo = inputDateTime?.subtract(Duration(days: 7));

  return sevenDaysAgo;
}

DateTime? sevenDaysAfter(DateTime? inputDateTime) {
  DateTime? sevenDaysMore = inputDateTime?.add(Duration(days: 7));

  return sevenDaysMore;
}
...
DocumentReference? getDocRef(
  ...
  if (collectionPath != null && documentId != null) {
...
DateTime? addThreeMonthsExpires(DateTime? dateTime) {
  if (dateTime == null) {
    return null; // Handle null input gracefully
  }
  return dateTime.add(Duration(days: 90));
}
